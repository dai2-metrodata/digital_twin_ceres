export function computeKPIs(params, processParams) {
  const { cuttingSpeed, feedRate, spindleSpeed, zeroPointCorrection, railLength } = params

  const speedRatio = cuttingSpeed / processParams.cuttingSpeed.max
  const feedRatio = feedRate / processParams.feedRate.max
  const spindleRatio = spindleSpeed / processParams.spindleSpeed.max
